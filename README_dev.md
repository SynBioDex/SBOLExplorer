...
Follow these steps in your WSL terminal. I'll wait — run each, and tell me if anything errors.

Step 1 — confirm Docker works

docker --version
docker ps
If docker ps errors with permission denied or "cannot connect to daemon", we need to fix that first — let me know what you see.

Step 2 — create a directory for Typesense's data

mkdir -p ~/typesense-data
This keeps your index on disk so it survives container restarts. (Don't use /tmp — WSL can clear it.)

Step 3 — pull and start Typesense
...
docker run -d \
  ...
  --api-key=xyz \
  --enable-cors
The xyz key matches what I put in flask/config.json — same key on both sides is required.

Step 4 — verify it's running

docker ps --filter name=typesense
curl http://localhost:8108/health
You should see the container listed as "Up" and the health check should return {"ok":true}.

Step 5 — confirm the API key works

curl http://localhost:8108/collections -H "x-typesense-api-key: xyz"
Should return [] (no collections yet — that's expected; we haven't created one).
